...
import { StickyMedia, type MediaItem } from './StickyMedia';
...
export function ScrollSteps({ steps, progressLabel = 'Progress' }: ScrollStepsProps) {
  ...
  const mediaItems = useMemo(() => steps.map((step) => step.media), [steps]);
          ...
          {steps.map((step, index) => {
            ...
            return (
              <section
                ...
                className={`relative min-h-[60vh] lg:min-h-[80vh] rounded-3xl border px-6 py-10 md:px-10 transition-all duration-300 ${
                  isActive
                    ? 'border-white/50 bg-white/[0.12] text-white shadow-[0_0_40px_rgba(251,191,36,0.2)]'
                    : 'border-white/20 bg-white/[0.08] text-zinc-300 opacity-70'
                }`}
              >
