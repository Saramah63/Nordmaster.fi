...
import { ArrowRight, Construction, Phone } from 'lucide-react';
...
export function generateMetadata({ params }: { params: { locale: string } }): Metadata {
  ...
  return {
    title: dict.meta.metalConstructionTitle,
    description: dict.meta.metalConstructionDescription,
    alternates: {
      languages: {
        en: '/en/services/metal-construction',
        fi: '/fi/services/metal-construction',
      },
    },
    openGraph: {
      title: dict.meta.metalConstructionTitle,
      description: dict.meta.metalConstructionDescription,
      url: `https://www.nordmastergroup.com/${locale}/services/metal-construction`,
    },
  };
}

export default function MetalConstructionPage({ params }: { params: { locale: string } }) {
  const { locale, dict } = getI18n(params);
  ...
  return (
    <div className="bg-zinc-950 text-zinc-100">
      <section className="relative overflow-hidden">
        <div className="container mx-auto px-4 py-16 lg:py-20">
          <div className="grid gap-12 lg:grid-cols-[1.1fr_0.9fr] items-center">
            <RevealOnScroll>
              <div>
                <div className="text-xs uppercase tracking-[0.4em] text-zinc-500">{dict.metalConstruction.eyebrow}</div>
                <h1 className="mt-4 text-4xl md:text-6xl font-semibold text-white">{dict.metalConstruction.title}</h1>
                <p className="mt-4 text-lg text-zinc-300 max-w-xl">{dict.metalConstruction.description}</p>
            ...
            <RevealOnScroll>
              <div className="relative aspect-[4/5] overflow-hidden rounded-[32px] border border-white/15 shadow-2xl shadow-black/60">
                <SafeImage
                  src="/images/metal/welding-04.jpg"
                  alt={dict.alts.metalConstructionHero}
      ...
      <Section
        eyebrow={dict.metalConstruction.includedEyebrow}
        title={dict.metalConstruction.includedTitle}
        description={dict.metalConstruction.includedDescription}
      >
        <div className="grid gap-6 lg:grid-cols-3">
          {dict.metalConstruction.items.map((item, index) => (
          ...
          <RevealOnScroll>
            <div className="rounded-[32px] border border-white/10 bg-amber-400 p-8 lg:p-12 text-zinc-950">
              <div className="grid gap-6 lg:grid-cols-[1.2fr_0.8fr] items-center">
                <div>
                  <div className="text-xs uppercase tracking-[0.4em] text-zinc-700">{dict.metalConstruction.ctaEyebrow}</div>
                  <h2 className="mt-3 text-3xl md:text-4xl font-semibold">{dict.metalConstruction.ctaTitle}</h2>
                  <p className="mt-4 text-sm text-zinc-800">{dict.metalConstruction.ctaBody}</p>
                </div>
                <div className="flex flex-wrap gap-4">
                  ...
                  >
                    {dict.metalConstruction.ctaOffer}
                    <ArrowRight size={16} />
                  </Link>
                  <a
                    href="tel:+358406206846"
                  ...
                  >
                    {dict.metalConstruction.ctaCall}
                  </a>
                </div>
              </div>
            </div>
